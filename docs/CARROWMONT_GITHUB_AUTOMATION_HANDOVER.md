...
- the completed Batch PR Publisher canonicalization and duplicate-workflow cleanup;
...
## 8. Batch PR Publisher canonical workflow - cleanup completed

The Batch PR Publisher housekeeping was completed on 26 September 2026.

The corrected complete-baseline publisher implementation is now stored under the single canonical filename:

```text
.github/workflows/carrowmont-batch-pr-publisher.yml
```

Temporary duplicate workflow files created during the Windows/browser rename sequence were removed:

```text
carrowmont-batch-pr-publisher (1).yml
carrowmont-batch-pr-publisher (2).yml
```

The canonical workflow retains the corrected behavior required for partial release bundles: it builds the complete Carrowmont QA baseline, validates the reviewed release bundle, runs staged QA, and creates review PRs only after the staged checks pass. It does not auto-merge normal product changes.

After the cleanup, a fresh **Carrowmont Source Snapshot** was run successfully and completed green with **1 artifact**, establishing a post-cleanup baseline.

### Current rule

When selecting **Carrowmont Batch PR Publisher** in GitHub Actions, the expected workflow file is simply:

```text
carrowmont-batch-pr-publisher.yml
```

If a future duplicate publisher file appears again, treat it as housekeeping drift and resolve it before using the publisher for a new release.
...
3. Verify the selected workflow file is the canonical `carrowmont-batch-pr-publisher.yml`.
...
| Source Snapshot workflow | Active; fresh post-cleanup snapshot green with 1 artifact on 26 Sep 2026 |
| Batch PR Publisher | Active under canonical `carrowmont-batch-pr-publisher.yml` |
...
| Duplicate Batch PR Publisher cleanup | Completed on 26 Sep 2026; `(1)` and `(2)` duplicates removed |
...
12. Confirm the Batch PR Publisher uses the canonical filename `carrowmont-batch-pr-publisher.yml`.
13. Never ask the user to paste a PAT/token into chat.

---

## 27. Housekeeping completion record - 26 September 2026

The planned automation housekeeping after the country/currency batch has been completed:

1. The updated automation handover was preserved in `docs/CARROWMONT_GITHUB_AUTOMATION_HANDOVER.md`.
2. The corrected complete-baseline Batch PR Publisher implementation was moved into the canonical filename:

```text
.github/workflows/carrowmont-batch-pr-publisher.yml
```

3. Temporary duplicate publisher workflows `(1).yml` and `(2).yml` were removed.
4. Only one canonical **Carrowmont Batch PR Publisher** remains in `.github/workflows/`.
5. A fresh Carrowmont Source Snapshot was run after cleanup and completed green with **1 artifact**.
6. Normal product changes remain PR-based and manually merged.
7. The canonical publisher should next be exercised with a legitimate reviewed release bundle, not a meaningless test change.

No further publisher-cleanup action is pending from this work session.
